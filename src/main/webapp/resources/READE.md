...
- AdminLTE(부트스트랩기반템플릿-반응형)를 이용해서커미 관리자단 디자인 만들기
...
#### 20210528(금) 작업예정
- UI트랜드용어: 카드(트럼프)UI, 뱃지(명찰), 리본(이벤트표시)
- 소스 인스펙션(소스분석);
...
#### 20210526(수) 작업
- 검색엔진에 최적화? 의미있는 태그를 사용했는가?<header>, <sectio>, <footer>, <article> 태그처럼 html5부터 만들어진 태그
- 유효성검사? validation 입력값이 제대로된 값인지 확인하는 과정.
- html5내장은 유효성검사? required(공백체크), type="email"(이메일형식체크)
, type= "password" (입력값을 숨김처리), type-"number"(숫자체크)
- js로 유효성검사? 비밀번호 확인(2개 입력값을 체크)
- 반응형 서브페이지들(로그인, 회원가입, 마이페이지).
- HTML5부터는 유효성검사가 내장되있기 때문에, JS로 빈칸인지 코팅할 필요가 없습니다. required 속성을 태그에 짚어넣으면 끝.
- 기획시, 메인 CSS 네이밍, 서브1 CSS 네이밍 ->기반으로 퍼블리셔, 프론트개발자 CSS나 HTML작업 이어서 합니다.
- 과장/이사급 UI를 2개파일 만듬 또는 테마포레스트 같은 디자인UIhtml+css+js를 구매) -> 기반으로 퍼블리셔, 프론트개발자 CSS나 HTML작업 이어서합니다
- 금액이 4000이상 2억 안팎인 프로젝트 자체UI 사용.
