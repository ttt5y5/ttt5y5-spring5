...
class Salesman extends Employee {//extends확장 -> 상속
   public Salesman() { //클래스명과 똑같은 이름의 메서드가 생성자 함수임.
      // 자동으로 생성되는데, 개발자가 커스터마이징 할 때, 만듭니다.(아래)
...
abstract class GraphicObject {
	int x,y;
	abstract void draw(); //구현내용이 없고, 메서드 명만 존재. 추상메서드입니다.
	// 추상메서드를 만드는 이유는 메서드가 수십개 수백개 일때 개발자가 구현할때 어려움이 존재
...
class Traiangle extends GraphicObject {
		//삼각형그리기
	@Override //부모크래스의 메서드를 재정의해서 = 오버라이드 / 오버로드  차이점확인(Step1.java 확인)
	void draw() {
		// 삼각형을 만듬
		System.out.println("  *  ");
		System.out.println(" * * ");
...
class Rectangle extends GraphicObject {
	...
	@Override
	void draw() { //오버라이드 @애노테이션이 있으면, 상속이라는 말.
