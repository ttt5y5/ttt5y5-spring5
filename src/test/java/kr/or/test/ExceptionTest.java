package kr.or.test;
/**
 * 이 클래스는 자바앱에서 개발자가 예외를 처리 하는 방법 실습하는 클래스 입니다.
 * @author 유지강
 *
 */
public class ExceptionTest {
	...
	public static void main(String[] args) {
		...
		// 위 처 동일 패키지안의 클래스는 import 없이 사용 가능합니다
		...
		for(int cnt=0;cnt<=3;cnt++) {
			/*
			 * 개발자가 예외처리하는 기본 형식 
			 * try {필수)구현프로그램 에러가 발생하면} 
			 * catch(에러발생 정의된 상황){필수)에러발생시 처리할구현내용}
			...
			indexValue = Integer.parseInt(stringArray[cnt]);
				...
				System.out.println(cnt + "번째 숫자형태가 올바르지 않습니다. 확인해 주세요!");
				System.out.println("에러내용은:" + ex.toString());
			} catch(ArrayIndexOutOfBoundsException ex) {
				System.out.println(ex.toString());
				System.out.println("에러내용은 배열의 배열의 크기가 올바르지 않습니다.");
			} finally {
