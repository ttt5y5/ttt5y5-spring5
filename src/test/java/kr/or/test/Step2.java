...
class MemberVO {
	...
	//하나씩 출력하기 힘들때 자동으로 멤버변수값을 추력하는 명령 toString생성
	
	@Override
	public String toString() {
		return "MemberVO 출력 [name=" + name + ", age=" + age + ", phoneNum=" + phoneNum + "]";
	}
	//위 프라이빗 멤버변수를 입출력을 구현 메서드를 만듭니다
	...
	// String 반환(return)=출력하는 겟 메서드 입니다. (아래)
	public String getPhoneNum() {
		return phoneNum;
	}
	// void(반환값이 없이 ) 입력하는 셋 메서드 입니다. (아래)
	public void setPhoneNum(String phoneNum) {
		this.phoneNum = phoneNum; //프라이빗변수를 직접접근을 못하니까
		// 퍼블릿 메서드로 입력하게 만들어 놓은 구조 입니다.
	}
...
public class Step2 {
	...
	public static void main(String[] args) {
		...
		memberService.printMember(members); //객체로 만들면, 호출(runtime)시 메모리에 로딩됩니다 -> 실행끝나면, 변환
		//외부클래스는 아래처럼 직접 접근해서 메서드나 변수를 사용할 수 없음
		//외부클래스 접근하려면, 객체(실행가능한=메모리로딩)로 만들어야 가능함
		//static 예약어는 컴파일시 메모리에 로딩(객체가됨)이 되게 명시
		//매번 static으로 만들면 메모리 가득차서 프로그램 실행이 느려지거나 멈춤현상발생
...
class MemberService {
	...
	public void printMember(MemberVO[] members) {
		// 멤버서브클래스에서 퍼블릿 접근제어로 회원정보를 출력하는 메서드 생성
		// 향상된for문을 사용합니다.
		for(MemberVO member:members) {
			System.out.println("회원의 이름은"+member.getName()
			+" 나이는 "+member.getAge() + " 폰번호는 "
			+member.getPhoneNum());
		}
	}
	//회원을 출력하는 메서드를 생성합니다.
