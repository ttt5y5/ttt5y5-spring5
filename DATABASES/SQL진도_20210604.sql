--19장 사용자추가시(CreateWorkSpace)시 오라클데스크탑(고전)X 대신 ,
-- 웹프로그램 사용 http:127.0.0.1:9000/ap
--SQL플러스X

--15장 PK생성시 자동으로 2가지가 생성 NOT NULL, UNIQUE(NO중복)
--제약조건(constraint)이 자동생성, Index(테이블)도 자동생성(검색시중요)
--ERD로 게시판테이블-[댓글|첨부파일] Foreign KEY(외래키)부자관계생성

--14장 트랜잭션 DB단에서 사용하지않고, 
--스프링단에서 트랜잭션을 사용 @Transactional 인터페이스를 사용
--commit;과 rollback;(DML문insert, update, delete)
--rollback는 제일 마지막 커밋된 상태로 되돌아갑니다.
...
--10장 테이블 조인 2개테이블 연결해서 결과를 구하는예약어
--댓글개수 구할때,
--카티시안프러덕트 조인(합집합-게시물10개, 댓글100= 110개~1000개)

--(인너)조일(교집합)을 제일 많이 사용
--아래조인방식이 오라클방식
SELECT dept.deptno, emp.* FROM emp, dept 
...
WHERE e.ename = 'SCOTT';
--JOIN과 GROUP 을 이용해서 댓글카운터도 출력하는 게시판리스트만들기
...
--부서별 평균 연봉이 2000 이상인 부서의 번호와 수버별평균급여(아래)
...
-- 라운드 함수(반올림) 소수점기준. round(10.05,2)소수점2째반올림
...
(SELECT round(AVG(sal))FROM emp); -- >많이 받는사람 <적게받는사람
--위쿼리는 사원중에 평균연봉보다 많이 받는 사람의 숫자. *error
-- 위 AVG 함수를 where조건에 사용하지 못할때 서브쿼리를 이용합니다.
SELECT MAX(sal)
, MIN(sal)
...
--DCL명령어 인 커밋이 필수입니다.
COMMIT; --데이터베이스쿼리직접입력한 결과는 반드시 커밋을 해줘야지만 실제 저장이 됩니다.
-- 커밋을 하지않으면 여기만보이고, 다른곳은 보이지 않습니다.
SELECT * FROM dept02;
--DELETE 는 레코드 한줄을 지우는 명령
DELETE FROM dept02; --이렇게 사용 하시면 모든레코드삭제됨 *주의요망.
...
UPDATE emp01 SET ename = '유지강' WHERE empno = 7839;
ROLLBACK; --DCL문 롤백은 마지막 커밋 바로전까지 되돌립니다.
